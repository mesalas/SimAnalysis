...
def make_macro_network_analysis(graph, clustering_method):
    ...
    total_vol = np.sum([graph[edge[0]][edge[1]]["weight"] for edge in graph.edges()])
    print(total_vol)
    ave_vol = list()
    ave_clique_vol = list()
    ...
    for clique,agents in clique_agents_map.items():
        ...
        clique_graph_unnormalized = graph.subgraph(agents) # make subgraph of clique with volumes
        clique_graph_normalized = normalized_graph.subgraph(agents) # make subgraph of clique with normalized volumes
        ...
        for agent in agents:
            vol += np.sum([data["weight"] for data in graph[agent].values()]) #sum volume traded by agent to other agents
            normalized_graph.nodes[agent]["community"] = clique #set the community property to the clique number
            relative_vol_within_clique.append(np.sum([data["weight"] for data in clique_graph_normalized[agent].values()]))
        ...
        vol_within_clique = np.sum([clique_graph_unnormalized[edge[0]][edge[1]]["weight"] for edge in clique_graph_unnormalized.edges()])
        print(vol_within_clique,vol_within_clique/total_vol)
        density = nx.density(clique_graph_unnormalized)
